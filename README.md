...
`agents/security-auditor`는 스킬이 아니라 **서브에이전트**이며 description상 보안 검토 작업에서 자동 활성화됩니다. 그래서 보안 요청에서는 리뷰 스킬과 동시에 매칭될 수 있지만, 층이 다르므로 경쟁이 아닙니다 — **어느 스킬을 실행할지는 위 우선순위가 결정합니다.** `branch-merge-review`의 팀 검토는 이 에이전트를 띄우지 않고 `references/reviewer-prompts.md`의 인라인 persona로 리뷰어를 구성해 `web-security-review`를 호출시킵니다. `security-auditor`는 사용자가 직접 보안 감사를 위임할 때 쓰는 독립 실행 단위이며, Claude에서는 `web-security-review`를 사전 로드하고 Codex에서는 배정받은 스킬을 호출해 같은 참조 기준으로 점검합니다. 발견과 개선 가이드만 산출하고 코드를 수정하지 않습니다.
...
읽기 전용 검토에 구현 관련 단어만 등장하면 `plan-and-build`를 억제합니다. 검토와 명시적인 구현 요청이 함께 감지되면 두 안내를 제공하고, 에이전트가 실제 요청과 범위별 권한을 판단합니다. 평범한 코드·보안·브랜치 리뷰, 체크포인트에 관한 설명 요청, 단순한 퇴근 인사에는 동작하지 않습니다. 훅은 안내만 제공하며 명령, 파일 변경, staging, commit, push를 실행하지 않습니다.
