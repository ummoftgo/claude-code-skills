...
|----|------------------|-----------|------|
| C1 | 신뢰상태-전달-누락 | 오케스트레이터가 `READ_ONLY`/`UNTRUSTED_DIFF`를 하위 리뷰어에 전달하지 않아 게이트가 기본 비활성 | open |
| H1 | 프롬프트-인젝션-경계 | raw diff가 지시와 같은 프롬프트 층에 놓여 데이터 전용 경계가 없음 | open |
| C2 | go테스트-레이스-허용 | 읽기 전용에서 `go test -race`를 명시적으로 허용 | open |
| C3 | cargo감사-설정-미점검 | cargo 설정 사전 점검 없이 `cargo audit` 실행 | open |
| M3 | go툴체인-고정-누락 | `GOTOOLCHAIN=local` 미지정으로 툴체인을 diff가 좌우할 수 있음 | open |
| H3 | 실행선택-파일-분류누락 | `.cargo/config`, `rust-toolchain.toml`, `go.work`, `*.jsx`, lockfile, `.npmrc` 미분류 | open |
| M1 | rename수집-NUL안전위반 | rename 수집이 `cut -f2`를 써서 NUL-safe 규칙과 모순 | open |
