...
>
> **A second axis — does the tool *execute* the code under review?** Not writing files and not
> running attacker-controlled code are different guarantees; the read-only flags above only buy
> the first. Several analysis tools run code the diff controls — a `build.rs` under `cargo clippy`,
> a flat ESLint config, a PHPStan `bootstrapFiles:` entry, an `.npmrc` that injects `node-options`.
> Before running any tool on a diff you would not execute, read
> [references/untrusted-execution.md](references/untrusted-execution.md): it lists which tools
> execute what, the resolution chain to check, the host-level isolation required, and the PHPStan
> policy (on an untrusted diff a PHPStan config at all is a stop). When a tool would run code from
> the diff and no isolation is available, record the check as `skipped-untrusted-execution` and say
> what it would have taken to run it. Your own team's branch is the ordinary case and needs none of
> this — but a newly introduced build hook, plugin, or dependency is code that was not there before
> and earns its own look regardless of who wrote it.
>
...
- `references/untrusted-execution.md` — which tools execute code the diff controls; read when `UNTRUSTED_DIFF=1` or provenance is unclear
...
--version` fails loudly instead. This holds on both platforms and leaves nothing in the
working directory. Everything after `--` belongs to the tool.
