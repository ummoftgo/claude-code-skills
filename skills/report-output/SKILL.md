...
- **Identifiers**: A report is human-facing output. When it refers to work by a short identifier (`A1`, `FR-001`, `CH-2`), invoke `readable-ids` if it is installed and write `A1(feature/label)` on first mention, short form afterwards. Where that skill is unavailable, keep a label beside the identifier anyway — a reader who has to open another document to learn what `A1` is has lost the summary.
