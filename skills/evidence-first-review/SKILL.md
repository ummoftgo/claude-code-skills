...
description: "Evidence-grounded read-only review that locks the user's context and scope, then independently verifies current code, diffs, files, raw JSON/CSV/database data, and runtime results. Use for a recheck of prior findings, a second or final review, a final approval or sign-off decision, verification of specific claims, or direct inspection of raw data or a non-Git directory — including when the scope is a PR or branch ('이 PR의 이전 지적을 재검토하고 최종 승인해줘'). An ordinary first-time PR or branch review belongs to branch-merge-review instead; a read-only constraint alone does not select this skill."
---
