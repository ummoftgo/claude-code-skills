...
읽기 전용 검토에 구현 관련 단어만 등장하면 `plan-and-build`를 억제합니다. 검토와 명시적인 구현 요청이 함께 감지되면 두 안내를 제공하고, 에이전트가 실제 요청과 범위별 권한을 판단합니다. 평범한 코드·보안·브랜치 리뷰, 체크포인트나 인수인계의 의미를 묻는 설명 요청, 단순한 퇴근 인사에는 동작하지 않습니다. 훅은 안내만 제공하며 명령, 파일 변경, staging, commit, push를 실행하지 않습니다. malformed JSON이나 지원하지 않는 입력도 프롬프트 처리를 막지 않습니다.
